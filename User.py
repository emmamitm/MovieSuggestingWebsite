...
class MoviesDB:
    def __init__(self, user):
        ...
        self.visualize_tree(self.root, 'bst')
    ...
    def visualize_tree(self, node, filename):
        dot = Digraph()
        self.add_nodes(dot, node)
        dot.render(filename, format='png', view=True)
    def add_nodes(self, graph, node):
        if node:
            graph.node(str(node.score), node.movie.title)
            if node.left:
                graph.edge(str(node.score), str(node.left.score))
                self.add_nodes(graph, node.left)
            if node.right:
                graph.edge(str(node.score), str(node.right.score))
                self.add_nodes(graph, node.right)
